...
#include "scullq.h"		/* local definitions */
...
irqreturn_t irq_handler(int irq, void *this) {
	...
	struct scull_dev *dev = (struct scull_dev *) this;
	struct timeval tv;
	int written;
	...
	BUG_ON(written != 16);
	printk(KERN_INFO "scullq: interrupt %d seen at %s\n", irq, (char *)short_head);
                ...
                loff_t *f_pos)
{
	...
	int count0 = short_head - short_tail;
...
static void scull_setup_cdev(struct scull_dev *dev, int index)
{
	...
	if (err)
		printk(KERN_NOTICE "Error %d adding scullq%d", err, index);
}
 ...
 */
	if (scull_major) {
		...
		result = register_chrdev_region(dev, scull_nr_devs, "scullq");
	} else {
	...
	scull_cache = kmem_cache_create("scullq",scull_quantum,0,SLAB_HWCACHE_ALIGN,NULL); /* no ctor/dtor */
	if (!scull_cache) {
		result = -ENOMEM;
		goto fail;
	}
	...
	short_buffer = __get_free_page(GFP_KERNEL); 
	short_head = short_tail = short_buffer;


	/* request irq */
 	result = request_irq (19, irq_handler, IRQF_SHARED, "scullq0", &scull_devices[0]);
  	if (result)
